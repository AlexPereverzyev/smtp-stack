...
class Mail extends Protocol {
    ...
    close() {
        ...
        if (this._session.envelope && this._session.envelope.dataStream) {
            this._session.envelope.dataStream.removeAllListeners();
            this._session.envelope.dataStream = null;
        }
    ...
    handleData(command, callback) {
        ...
        this._session.envelope.dataStream = this._parser.startData(this._settings.size);

        this.emit('data', this._session, (err, message) => {
            // do not accept message until the stream is consumed
            if (this._session.envelope.dataStream && this._session.envelope.dataStream.readable) {
                this._session.envelope.dataStream.once('end', () => this._acceptMail(err, message));
                return;
            }

            this._acceptMail(err, message);
        });
    ...
    _acceptMail(err, message) {
        const code = err
            ? err.code || consts.StatusCodes.MailboxUnavailable
            : consts.StatusCodes.Accepted;
