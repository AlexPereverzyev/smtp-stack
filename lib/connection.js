...
class SmtpConnection extends EventEmitter {
    constructor(manager, settings, socket, socketSettings) {
        ...
        this._stack = [
            new Auth(this._settings, this.session)
                ...
                .on('auth', (s, c) => this.emit('auth', s, c)),
            ...
            new Mail(this._settings, this.session, this._parser)
                .on('send', (c, m) => this.send(c, m))
                .on('mail', (s, c) => this.emit('mail', s, c))
                .on('rcpt', (s, c) => this.emit('rcpt', s, c))
                .on('data', (s, c) => this.emit('data', s, c))
                .on('reset', () => this._resetSession()),
            new XClient(this._settings, this.session)
                .on('send', (c, m) => this.send(c, m))
                .on('login', (s, c) => this.emit('auth', s, c))
                .on('state', (u) => this._updateState(u))
                .on('close', () => this.close()),
    ...
    send(code, data) {
        ...
        if (code === consts.StatusCodes.ServiceUnavailable) {
            this.close();
        }
    }
    ...
    _onTimeout() {
        this.send(consts.StatusCodes.ServiceUnavailable, 'Connection idle timeout');
    }
    ...
    _resetSession() {
        ...
        this.session.envelope.lastTo = null;

        if (this.session.envelope.dataStream) {
            this.session.envelope.dataStream = null;
        }
    ...
    _onCommand(command, callback = function () {}) {
        ...
        logger.debug({
            ...
            command: commandName,
            message: 'received',
            args: commandArgs && commandArgs.length ? commandArgs : undefined,
        });
        ...
        if (!this._ready) {
            this.send(
                consts.StatusCodes.ServiceUnavailable,
                this._settings.name + ' Wait to be ready'
            );
            return;
        }
    ...
    _smtpMiddleware(ctx, next) {
        let handler;
        for (const protocol of this._stack) {
