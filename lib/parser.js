...
class SmtpStream extends Writable {
    ...
    startData(maxBytes) {
        this._maxBytes = Number(maxBytes) || Infinity;
        this._data = true;
    ...
    _write(chunk, encoding, next) {
        ...
        if (this._data) {
            this._feedData(chunk, done);
        } else {
            this._feedCommand(chunk, done);
        }
    }

    _feedCommand(chunk, done) {
        const data = Buffer.concat(
            [this._remainderCmd, chunk],
            this._remainderCmd.length + chunk.length
        );

        let pos = 0;

        // todo: handle error from the callback?
        const readLine = (_) => {
    ...
    _feedData(chunk, done) {
        let data = Buffer.concat(
            [this._remainderDta, chunk],
            this._remainderDta.length + chunk.length
        );
    ...
    _endData(lastChunk, commandPart, callback) {
        // write next command bytes as soon as parser is switched back to command
        if (commandPart && commandPart.length) {
            this._continueCallback = () => this._write(commandPart, 'buffer', callback);
        }
