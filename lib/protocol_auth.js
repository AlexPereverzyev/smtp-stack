...
class Authentication extends Protocol {
    ...
    handlePlainToken(token, callback) {
        ...
        if (payload.length !== 3) {
            this.emit('send', 500, 'Error: AUTH PLAIN payload');
            callback();
            return;
        }
    ...
    handleLogin(args, callback) {
        if (args.length > 1) {
            this.emit('send', 5001, 'Error: AUTH LOGIN syntax');
            callback();
            return;
        }
    ...
    handleLoginUsername(username, callback) {
        ...
        if (!username) {
            this.emit('send', 500, 'Error: username missing');
            callback();
            return;
        }
    ...
    handleLoginPassword(username, password, callback) {
        ...
        if (!password) {
            this.emit('send', 500, 'Error: password missing');
            callback();
            return;
        }
    ...
    handleXOAuth2Token(token, callback) {
        ...
        if (!(username && accessToken)) {
            this.emit('send', 500, 'Error: invalid XOAUTH2 payload');
            callback();
            return;
        }
    ...
    handleCramMd5Token(challenge, token, callback) {
        ...
        if (!(username && challengeResponse)) {
            this.emit('send', 500, 'Error: challenge response missing');
            callback();
            return;
        }
