...
class Authentication extends Protocol {
    ...
    handlePlainToken(token, callback) {
        ...
        this.emit('auth', this._session, (err, result) =>
            this._handleAuthResult(err, result, callback)
        );
    }
    ...
    handleCramMd5(args, callback) {
        ...
        const hostname = this._settings.name;
        const challenge = `<${random}${timestamp()}@${hostname}>`;
    ...
    _handleAuthResult(err, result, callback) {
        if (err) {
            logger.error({
                err,
                sid: this._session.id,
                command: Commands.AUTH,
                method: this._session.auth.method,
                username: this._session.auth.username,
                message: err.message,
            });
        ...
        logger.debug({
            ...
            silent: this._settings.noAuthReply,
        });

        this._session.user = result.user;

        if (this._settings.noAuthReply) {
            callback();
            return;
        }
